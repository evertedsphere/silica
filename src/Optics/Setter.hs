...
-- @
(?~) :: AsSetter k => Optic k s t a (Maybe b) -> b -> s -> t
l ?~ b = set l (Just b)
{-# INLINE (?~) #-}
...
-- @
(<.~) :: AsSetter k => Optic k s t a b -> b -> s -> (b, t)
l <.~ b = \s -> (b, set l b s)
{-# INLINE (<.~) #-}
...
-- @
(<?~) :: AsSetter k => Optic k s t a (Maybe b) -> b -> s -> (b, t)
...
-- @
(+~) :: (Num a, AsSetter k) => Optic k s t a a -> a -> s -> t
l +~ n = over l (+ n)
{-# INLINE (+~) #-}
...
-- @
(//~) :: (Fractional a, AsSetter k) => Optic k s t a a -> a -> s -> t
l //~ n = over l (/ n)
{-# INLINE (//~) #-}
...
-- @
(^~) :: (Num a, Integral e, AsSetter k) => Optic k s t a a -> e -> s -> t
l ^~ n = over l (^ n)
{-# INLINE (^~) #-}
...
-- @
--
(^^~) :: (Fractional a, Integral e, AsSetter k) => Optic k s t a a -> e -> s -> t
l ^^~ n = over l (^^ n)
{-# INLINE (^^~) #-}
...
-- @
(**~) :: (Floating a, AsSetter k) => Optic k s t a a -> a -> s -> t
l **~ n = over l (** n)
{-# INLINE (**~) #-}
...
-- @
(||~):: AsSetter k => Optic k s t Bool Bool -> Bool -> s -> t
l ||~ n = over l (|| n)
{-# INLINE (||~) #-}
...
-- @
assign :: (MonadState s m, AsSetter k) => Optic k s s a b -> b -> m ()
...
(.=) :: (MonadState s m, AsSetter k) => Optic k s s a b -> b -> m ()
...
-- @
(%=) :: (MonadState s m, AsSetter k) => Optic k s s a b -> (a -> b) -> m ()
l %= f = State.modify (l %~ f)
{-# INLINE (%=) #-}

-- | This is an alias for ('%=').
modifying :: (MonadState s m, AsSetter k) => Optic k s s a b -> (a -> b) -> m ()
...
-- @
(?=) :: (MonadState s m, AsSetter k) => Optic k s s a (Maybe b) -> b -> m ()
l ?= b = State.modify (l ?~ b)
{-# INLINE (?=) #-}
...
-- @
(+=) :: (MonadState s m, Num a, AsSetter k) => Optic' k s a -> a -> m ()
l += b = State.modify (l +~ b)
{-# INLINE (+=) #-}
...
-- @
(//=) :: (MonadState s m, Fractional a, AsSetter k) => Optic' k s a -> a -> m ()
...
-- @
(^=) :: (MonadState s m, Num a, Integral e, AsSetter k) => Optic' k s a -> e -> m ()
...
-- @
(^^=) :: (MonadState s m, Fractional a, Integral e, AsSetter k) => Optic' k s a -> e -> m ()
l ^^= e = State.modify (l ^^~ e)
{-# INLINE (^^=) #-}
...
-- @
(**=) :: (MonadState s m, Floating a, AsSetter k) => Optic' k s a -> a -> m ()
l **= a = State.modify (l **~ a)
{-# INLINE (**=) #-}
...
-- @
(&&=):: (MonadState s m, AsSetter k) => Optic' k s Bool -> Bool -> m ()
l &&= b = State.modify (l &&~ b)
{-# INLINE (&&=) #-}
...
(<~) :: (MonadState s m, AsSetter k) => Optic k s s a b -> m b -> m ()
l <~ mb = mb >>= (l .=)
{-# INLINE (<~) #-}
...
-- @
(<.=) :: (MonadState s m, AsSetter k) => Optic k s s a b -> b -> m b
l <.= b = do
  l .= b
  return b
{-# INLINE (<.=) #-}
...
-- @
(<?=) :: (MonadState s m, AsSetter k) => Optic k s s a (Maybe b) -> b -> m b
l <?= b = do
  l ?= b
  return b
{-# INLINE (<?=) #-}
...
-- @
(<>~) :: (Monoid a, AsSetter k) => Optic k s t a a -> a -> s -> t
...
-- @
(<>=) :: (MonadState s m, Monoid a, AsSetter k) => Optic' k s a -> a -> m ()
l <>= a = State.modify (l <>~ a)
{-# INLINE (<>=) #-}
...
scribe :: (MonadWriter t m, Monoid s, AsSetter k) => Optic k s t a b -> b -> m ()
...
passing :: (MonadWriter w m, AsSetter k) => Optic k w w u v -> m (a, u -> v) -> m a
passing l m = pass $ do
  (a, uv) <- m
  return (a, over l uv)
{-# INLINE passing #-}
...
-- 'IndexedSetter'.
ipassing :: (MonadWriter w m, AsIndexedSetter i k) => Optic k w w u v -> m (a, i -> u -> v) -> m a
...
-- portion of the resulting 'MonadWriter'.
censoring :: (MonadWriter w m, AsSetter k) => Optic k w w u v -> (u -> v) -> m a -> m a
...
-- portion of the resulting 'MonadWriter', with access to the index of an
-- 'IndexedSetter'.
icensoring :: (MonadWriter w m, AsIndexedSetter i k) => Optic k w w u v -> (i -> u -> v) -> m a -> m a
...
-- @
iset :: (AsIndexedSetter i k) => Optic k s t a b -> (i -> b) -> s -> t
...
-- @
(%@~) :: AsIndexedSetter i k => Optic k s t a b -> (i -> a -> b) -> s -> t
(%@~) = iover
{-# INLINE (%@~) #-}
...
-- @
(%@=) :: (MonadState s m, AsIndexedSetter i k) => Optic k s s a b -> (i -> a -> b) -> m ()
l %@= f = State.modify (l %@~ f)
{-# INLINE (%@=) #-}

-- | This is an alias for ('%@=').
imodifying :: (MonadState s m, AsIndexedSetter i k) => Optic k s s a b -> (i -> a -> b) -> m ()
...
-- @
(.@=) :: (MonadState s m, AsIndexedSetter i k) => Optic k s s a b -> (i -> b) -> m ()
l .@= f = State.modify (l .@~ f)
{-# INLINE (.@=) #-}
...
-- @
assignA :: (Arrow p, AsSetter k) => Optic k s t a b -> p s b -> p s t
...
mapOf :: AsSetter k => Optic k s t a b -> (a -> b) -> s -> t
mapOf = over
{-# INLINE mapOf #-}
...
-- @
imapOf :: AsIndexedSetter i k => Optic k s t a b -> (i -> a -> b) -> s -> t
imapOf = iover
{-# INLINE imapOf #-}
